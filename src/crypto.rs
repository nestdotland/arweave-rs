...
/// Generate a JWK Wallet.
pub async fn generate_jwk() -> Result<JwkPrivate, rsa::errors::Error> {
    let exponent = rsa::BigUint::from_bytes_be(&[0x01, 0x00, 0x01]);
    let mut rng = OsRng;
    let private_key: RSAPrivateKey =
        tokio::task::spawn_blocking(move || -> Result<RSAPrivateKey, rsa::errors::Error> {
            Ok(RSAPrivateKey::new_with_exp(&mut rng, 4096, &exponent)?)
        })
        .await
        .unwrap()?;
    Ok(JwkPrivate::from_rsa(private_key))
}
...
#[cfg(test)]
mod tests {
    use crate::crypto::{generate_jwk, Crypto, CryptoInterface, Sha256};
